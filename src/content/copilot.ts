// Copilot-specific helpers: query Shadow DOM, flatten nested templates,
// and extract assistant/user messages from the Copilot/Bing UI.

/**
 * Helper to query elements deeply through Shadow DOM boundaries.
 * Recursively enters shadow roots to find all matching elements.
 */
...
// Find all Copilot assistant message containers in the page (including Shadow DOM).
...
// Find all Copilot user message containers in the page (including Shadow DOM).
 ...
 * This flattens Shadow DOM boundaries so standard DOM APIs (querySelector,
 * textContent, Turndown) can read the content.
 */
...
// Return a flattened clone of a Copilot message so standard content extraction works.
